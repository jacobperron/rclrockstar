# Original source: https://github.com/yyyyyyyan/rockstar-py/blob/main/rockstarpy/transpile.py
# Credit to yyyyyyyan and contributors!
...
class Transpiler:
    ...
    REGEX_VARIABLES = r"(?:{}|{}|{})".format(
        COMMON_VARIABLE_FMT, PROPER_VARIABLE_FMT, SIMPLE_VARIABLE_FMT
    )
    QUOTE_STR_FMT = r"\"[^\"]*\""
        ...
        if match and match.group(2).split()[0] not in poetic_type_literals_keywords:
            ...
            for word_number in match.group(2).split():
                if re.match(r"\d+", word_number):
                    ...
                    alpha_word_match = re.search(r"[A-Za-z]+", word_number)
                    if alpha_word_match:
                        line += str(len(alpha_word_match.group(0)) % 10) + period
                    else:
                        line += str(word_number)
        return line
    ...
    def transpile_line(self, line):
        if line == "\n":
            ...
            py_line = re.sub(
                r"Put (.*) (?:into|in) ({})".format(self.REGEX_VARIABLES),
                r"\g<2> = \g<1>",
                py_line,
            )
            py_line = re.sub(
                r"Let ({0}) be (.+)".format(self.REGEX_VARIABLES),
                r"\g<1> = \g<2>",
                py_line,
            )
            ...
            py_line = re.sub(
                r"({0}) (?:taking|wants) ((?:{0}|\"[^\"]*\"|[0-9]+)(?:(?:,| and) ?(?:{0}|\"[^\"]*\"|[0-9]+))*)".format(
                    self.REGEX_VARIABLES
                ),
                r"\g<1>(\g<2>)",
                py_line,
...
def transpile(rockstar_file):
    transpiler = Transpiler() 
    output = []
    line_number = 1 
    with open(rockstar_file, 'rb') as lyrics:
        for line in lyrics:
            line = line.decode('utf8')
            try:
                output.append(transpiler.transpile_line(line))
